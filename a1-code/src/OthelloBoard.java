...
public class OthelloBoard {
	...
	private boolean validCoordinate(int row, int col) {
		// TODO: Complete this method (Task 1.1)
		return 0 <= row && row < this.getDimension() && 0<= col && col < this.getDimension();
	...
	private char checkForPattern(int row, int col, int drow, int dcol) {
		...
		if (this.validCoordinate(i, j)) {
			...
			if (this.board[i][j]== P1 ) {
				while(this.validCoordinate(i+drow, j+dcol)) {
					if(this.board[i+drow][j+dcol] == EMPTY) {
						return EMPTY;
					}
					if(this.board[i+drow][j+dcol] != P1) {
						return P2;
					}
					i += drow;
					j += dcol;
			...
			if (this.board[i][j]== P2 ) {
				while (this.validCoordinate(i+drow, j+dcol)) {
					if(this.board[i+drow][j+dcol] == EMPTY) {
						return EMPTY;
					}
					if(this.board[i+drow][j+dcol] != P2) {
						return P1;
					}
					i += drow;
					j += dcol;
	...
	private int flip(int row, int col, int drow, int dcol, char player) {
		...
		if(this.board[row][col] == EMPTY) return -1;
		
		int i = row ;
		int j = col ;
		int n = 0;
		while( this.validCoordinate(i, j) && this.board[i][j] != player) {
			this.board[i][j] = player;
			i += drow;
			j += dcol;
			n++;
			}
		if (!this.validCoordinate(i, j)) return -1;
		return n;
	...
	public boolean move(int row, int col, char player) {
		...
		for (int i = -1; i <2; i++) {
			for (int j =-1; j <2; j++) {
				if(i != 0 || j != 0) {
					numflip = flip(row+i, col+j, i, j, player);
					if (numflip>0){
						Totalflip +=numflip;
					}			
				}
